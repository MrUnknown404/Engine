...
namespace Engine3.Graphics.Vulkan {
	public unsafe class VkWindow : Window {
		...
		public VkWindow(GameClient gameClient, VkInstance vkInstance, string title, uint width, uint height) : base(gameClient, title, width, height) {
			...
			LogicalGpu = new(SelectedGpu, gameClient.GetAllRequiredDeviceExtensions(), gameClient.GetAllRequiredValidationLayers());
		...
		[MustUseReturnValue]
		private static PhysicalGpu PickBestGpu(PhysicalGpu[] physicalGpus, GameClient.RateGpuSuitabilityDelegate rateGpuSuitability) {
