...
public static class LoggerH {
	...
	public static bool PrintToLogFile {
		get;
		set {
			if (wasSetup) { Logger.Warn($"{nameof(PrintToLogFile)} must be set before #{nameof(Setup)} is called"); }
			field = value;
		}
	} = true;
	...
	public static bool PrintToConsole {
		get;
		set {
			if (wasSetup) { Logger.Warn($"{nameof(PrintToConsole)} must be set before #{nameof(Setup)} is called"); }
			field = value;
		}
	} = true;
	...
	internal static void Setup() {
		...
		LogManager.Setup() /*.SetupLogFactory(static s => {
			...
			if (PrintToLogFile) { b.ForLogger().FilterMinLevel(FileLogLevel).WriteToFile(fileName: $"{LogFolder}/{DateTime.Now.ToString(LogDateFormat)}.{LogFileType}", layout: LogLayout, maxArchiveFiles: MaxFiles - 1); }
		});
