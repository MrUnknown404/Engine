...
public unsafe class VulkanWindow : Window {
	...
	public VulkanWindow(VulkanBackend backend, string title, uint width, uint height) : base(backend, title, width, height) {
		...
		SelectedGpu = PickBestGpu(backend.Settings, availableGpus, backend.RateGpuSuitability);
	...
	[MustUseReturnValue]
	private static SurfaceCapablePhysicalGpu PickBestGpu(VulkanSettings settings, SurfaceCapablePhysicalGpu[] physicalGpus, VulkanBackend.RateGpuSuitabilityDelegate rateGpuSuitability) {
		...
		foreach (SurfaceCapablePhysicalGpu device in physicalGpus) {
			int score = rateGpuSuitability(settings, device);
