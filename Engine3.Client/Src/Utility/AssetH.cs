...
public static class AssetH {
	...
	public static StbiImage LoadImage(string fileLocation, string fileType, byte texChannels, Assembly assembly) {
		...
		if (textureStream == null) {
			...
			textureStream = GetAssetStream($"Textures.{MissingTextureName}", Core.Engine3.Engine.Assembly) ?? throw new NullReferenceException("Could not find default texture");
		}
	...
	public static Model LoadModel(string fileLocation, [RequireStaticDelegate] GLTFMeshToMeshDelegate gltfMeshToMesh, Assembly assembly, byte vertexStride, GLTFFileType fileType = GLTFFileType.GLB) {
		...
		return new(Enumerable.ToArray(meshDecoders.AsValueEnumerable().Select(static meshDecoder => meshDecoder.Primitives).SelectMany(pList => pList.Select(primitiveDecoder => gltfMeshToMesh(primitiveDecoder))).ToArray()), vertexStride);
	}
