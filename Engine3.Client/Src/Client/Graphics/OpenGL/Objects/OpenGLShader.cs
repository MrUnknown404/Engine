...
namespace Engine3.Client.Client.Graphics.OpenGL.Objects;
...
public sealed class OpenGLShader : NamedGraphicsResource<OpenGLShader, nint> { // TODO add support for non vertex pulling shaders
	...
	internal OpenGLShader(string debugName, string fileLocation, ShaderType shaderType, Assembly assembly) : base(debugName) {
		...
		string fullFileName = $"{_3DGraphicsApi.OpenGL}.{fileLocation}.{shaderType.FileExtension}.{ShaderLanguage.Glsl.FileExtension}"; // TODO add spirv support https://wikis.khronos.org/opengl/SPIR-V
