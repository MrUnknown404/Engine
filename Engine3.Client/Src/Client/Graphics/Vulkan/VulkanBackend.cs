...
public sealed unsafe class VulkanBackend : _3DGraphicsBackend {
	...
	public VulkanBackend(VulkanGraphicsApiHints graphicsApiHints) : base(graphicsApiHints) { }
	...
	protected internal override void Setup(EngineGame game) {
		...
		VkInstance = CreateVulkanInstance(game.Name, game.Version);
		...
		PhysicalGpus = GetPhysicalGpus(Settings, VkInstance.Value, IsPhysicalDeviceSuitable, GetAllRequiredDeviceExtensions());
		Logger.Debug("Created Physical Gpus");
		PrintPhysicalGpus(Core.Engine3.Debug);
	}

	public string[] GetAllRequiredValidationLayers() {
		HashSet<string> allValidationLayers = new();
		allValidationLayers.UnionWith(Engine3Client.VulkanDefaults.RequiredValidationLayers);
	...
	public string[] GetAllRequiredInstanceExtensions() {
		...
		allInstanceExtensions.UnionWith(Engine3Client.VulkanDefaults.RequiredInstanceExtensions);
	...
	public string[] GetAllRequiredDeviceExtensions() {
		...
		allDeviceExtensions.UnionWith(Engine3Client.VulkanDefaults.RequiredDeviceExtensions);
	...
	private VkInstance CreateVulkanInstance(string name, IPackableVersion version) {
		...
		VkApplicationInfo applicationInfo = new() {
				...
				pEngineName = (byte*)Unsafe.AsPointer(ref MemoryMarshal.GetReference(Encoding.UTF8.GetBytes(Core.Engine3.Name))),
				engineVersion = Core.Engine3.Engine.Version.Packed,
	...
	[MustUseReturnValue]
	private static PhysicalGpu[] GetPhysicalGpus(VulkanSettings settings, VkInstance vkInstance, IsPhysicalDeviceSuitableDelegate isPhysicalDeviceSuitable, string[] requiredDeviceExtensions) {
		uint deviceCount;
		...
		foreach (VkPhysicalDevice physicalDevice in physicalDevices) {
			...
			if (!isPhysicalDeviceSuitable(settings, physicalDeviceProperties2.properties, physicalDeviceFeatures2.features)) { continue; }
